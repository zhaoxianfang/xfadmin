...
## v2.2.14 DIY 可视化布局器：预览关闭按钮修复与体验优化（2026-09-17）

- **修复预览浮层关闭按钮在窄屏被裁切**：预览栏原为单行 `flex` 且不换行，设备切换 + 宽度输入 + 关闭按钮总宽超出视口时，关闭按钮被挤出屏幕右侧，用户无法返回舞台。改为 `flex-wrap: wrap`，并让关闭按钮 `flex: 0 0 auto; margin-left: auto` 始终贴右、换行后也可见；按钮文案改为「← 退出预览」（带返回箭头图标），按 **Esc** 亦可关闭。
- **预览浮层观感增强**：平板 / 手机预览画布增加设备级边框与圆角、加强阴影，更贴近真实设备；桌面预览保持铺满。
- **图表 / 地图占位说明**：编辑态舞台内图表 / 地图以静态占位呈现（标注「预览中查看真实渲染效果」），预览浮层 / 导出页按真实配置渲染，避免异步渲染节点被重渲染替换导致「永不显示」。
- **文档**：扩展 `diyLayoutPage` 组件章节，补充核心能力表、预览关闭 / Esc 返回、图表占位行为、示例布局与最新渲染骨架说明。

## v2.2.13 安全加固 / 前端缺陷修复 / 工具条接线 / 静态审计体系（2026-09-07）

本轮为全包体检，覆盖 216 个组件 + `xfadmin.js` / `xfadmin.css`，修复真实缺陷并新增可复跑的审计防线。

### 安全修复（注入类）

- **新增 CSS 值安全助手**（`Component` 基类）：`cssLen()` / `cssRatio()` / `cssColor()` / `cssBackground()` / `gridCol()`。
  `Html::e()` 不转义 `;` `:` `(` `)`，仅靠它无法阻止在 `style="..."` 里追加声明（点击劫持/覆盖层）。
  已接入 `ChatBox.height`、`Scrollspy.height`、`Carousel.height`、`Gallery.ratio`、`ProfilePage.cover`。
- **新增 `Component::text()`**：「可传组件实例、但传字符串必须转义」的槽位渲染，用于
  `Footer.text`、`Page.page_title`（字符串形态）、`Offcanvas.title`、`Collapse.trigger`、
  `LockScreen.copyright`、`SearchResults.pagination`、`ReportPage.filters/table`、`DashboardGrid.bottom`。
- **栅格断点白名单**：`StatCard.width` / `FileManager.cols` 的数组**键名**此前直接拼进 `class`，
  可注入 `x" onload=...`；现统一走 `ENUM_BREAKPOINT`。`ProductsGrid.columns` / `Marketplace.columns` 列数夹紧 1~6
  （顺带修正 `Marketplace` 的 `row-col-1` → `row-cols-1` 笔误）。
- **属性/文本转义补齐**：`AuthPage` 的 `class` 与 `buttons[].type/variant`、`brand.logo`；
  `NotificationCenter` / `I18n` 修正「先 `e()` 后拼 URL」导致查询串 `&` 被实体化的功能缺陷；
  `FieldWrapper.help`、`Layout\EmptyState.text`、`ProjectActivity.desc`、`ProductCard.price/old_price`、
  `ProductsGrid`/`Marketplace` 的 `currency` 由 `raw()` 改为转义；
  `Pagination.size`、`UI\EmptyState.size` 走 `enum()` 白名单。

### 健壮性修复（PHP 致命错误）

- `FormElements`：`cols=0` 时 `12 / min(0,4)` 触发 `DivisionByZeroError` → `max(1, ...)` 兜底。
- `CommandPalette`：`commands` 混入标量项时 `$c['label']` 触发 TypeError → 标量容错（与 Wizard/Tabs/Dropdown 一致）。
- `StatMiniSparkline`：传 `delta` 时涨跌箭头覆盖用户配置的 `icon` → 改用独立变量。

### 前端修复（`resources/assets/js/xfadmin.js`）

- **看板持久化此前完全失效**：`XFAdmin.request(url, payload, {method})` 签名错误（第二参才是 opts），
  且回调里 `card` 未定义被 `catch` 静默吞掉 → 改为 `request(url, {method:'PATCH', data})` 并透传卡片节点。
- **全站打印白屏**：`@media print { body * {visibility:hidden} }` 在无 `.xf-invoice-print-area` 的页面会让
  Ctrl+P 得到完全空白 → 作用域限定为 `body.xf-printing`（由打印按钮添加、`afterprint` 移除）；
  同时修复打印按钮读的是 PHP 从未输出的 `data-target`（实际在 `data-xf-config.target`）。
- **XSS 汇点**：`countdown` 的 `expired` 文案、`timeline` 节点的 `btn.textContent`、`promptFields` 的
  `label`/`placeholder` 进 `innerHTML` 前统一 `escapeHtml()`。
- **`countdown` 定时器泄漏**：目标时间非法时仍每秒重写 DOM → 非法时不再注册定时器。
- **弹窗 backdrop 残留**：`dialog`/`formDialog`/`pageDialog` 直接 `el.remove()` 不触发 `hidden.bs.modal`，
  `.modal-backdrop` 与 `body.modal-open` 永久残留 → 新增 `XFAdmin.disposeModal(id)` 统一 dispose + 清理遮罩。
- **`scan()` 增量初始化不完整**：只调 `bindRemoteForms`，AJAX 注入内容的 tooltip/popover/表单校验失效 → 补 `initBootstrapExtras(root)`。
- **初始化失败不再重试**：`__xfInited` 在 try 前置位，插件 JS 未就绪时组件永久空白 → 改为成功后置位。
- **行分组不再强依赖 jQuery**：`drawCallback` 里裸调 `global.jQuery(...)`，DataTables 2 无 jQuery 时每次绘制抛错 → 原生 `insertAdjacentHTML` 优先。
- `popoverConfirm` 的 document 捕获监听只在「点卡片外」解绑 → 移入 `clean()` 统一解绑。
- `apexchart` 深拷贝补 try/catch；`initCaptcha` 选择器属性改用 `CSS.escape`。
- **preloader 兜底前置**：`#preloader` 是不透明全屏遮罩，其隐藏逻辑原在文件末尾，前面任一顶层异常即导致整页不可用 → 提到文件最前面独立 IIFE；`z-index` 由 99999 降到 1090。

### 新增能力

- **列表工具条接线**：`DataTableToolbar` 的 `dt-search` / `dt-filter` / `dt-pagesize` / `dt-views`
  此前在 JS 侧完全没有实现（死属性）→ 新增四个 widget：搜索防抖重绘、列筛选（`data-field`）、
  `page.len()`、视图切换（同步 active 并派发 `xf.dtview.change`）。新增 `table` / `filterField` 选项。
- **组件销毁钩子**：`XFAdmin.register(name, initFn, destroyFn)` 支持第三参；
  新增 `XFAdmin.destroy(el)` 与 `XFAdmin.destroyWithin(root)`（`countdown`/`countup`/`backtotop` 已提供 destroy），
  AJAX 替换内容前可显式清理定时器与全局监听；`DataTable` 在 `destroy.dt` 时反注册 ResizeObserver。
- `.select2-dropdown` z-index 由 1060 降到 1049（低于 Bootstrap 模态框 1055，原会穿越遮罩）。
- 删除 `xfadmin.css` 中重复的 `.auth-split-*` 定义（两份暗色兜底色不一致）与对 Bootstrap `.min-vh-100` 的无意义重定义。

### 质量基建

- 新增 `tests/audit.php`：四道防线（资源完整性 / 注入模糊审计 / 健壮性 / 前端契约），零依赖离线运行。
- 新增 `tests/run-all.sh`：PHP 全量语法检查 + JS 语法检查 + audit + smoke + regression，退出码 0/1。
- 文档：`docs/security.md` 新增 CSS 值安全助手与 `text()` 说明；`docs/TESTING.md` 新增静态审计章节。

### 验证

- `bash tests/run-all.sh`：PHP 语法 228 文件 PASS、JS 语法 PASS、审计 46 项 PASS、smoke / regression 全 PASS。
