...
- **3 种布局**：`base`（居中单卡） / `card`（左表单 + 右宣传图） / `split`（左整幅背景图 + 右表单）。
- **9 种语义类型**（`type`）：`sign-in` `sign-up` `reset-pass` `new-pass` `lock-screen` `login-pin` `two-factor` `delete-account` `success-mail`，各内置对应表单与文案。
- **侧栏毛玻璃卡片**：`card` / `split` 共用 `sideTitle` / `sideText` / `sideList` / `sideVariant` / `sideImage*` / `sideOverlay`。
- **`sideImage` 四种写法**：① 包内图片名 `'auth.jpg'` ② 外部 URL `'https://...'` ③ data URI ④ 本地公开路径 `'/uploads/bg.jpg'`（`''`/`false`/`null` 关闭 → 纯色渐变）。
...
| 键 | 默认值 | 说明 |
|----|--------|------|
| `type` | `'sign-in'` | 语义类型（见上） |
| `layout` | `'base'` | `base` \| `card` \| `split` |
| `theme` | `'light'` | `light` \| `dark`（侧栏风格） |
| `brand` | `['name'=>'XfAdmin','url'=>'/','logo'=>null]` | `logo` 可为图片地址 |
| `heading` / `subheading` | `''` | 主 / 副标题 |
| `fields` | `[]` | 覆盖/追加默认表单字段（关联数组） |
| `submit` | `'提交'` | 提交按钮文案；或 `['text','icon','variant','class']` |
| `content` | `''` | 自定义整块主体（覆盖默认表单） |
| `prepend` / `append` | `''` | 标题后 / 提交按钮前插入（raw） |
| `beforeForm` / `afterForm` | `''` | `<form>` 前 / 后插入（raw） |
| `links` | `[]` | 导航链接 `['text','href']` |
| `captcha` | `false` | `false` 不渲染 / `true` 占位 / 字符串原样输出 |
| `socialButtons` | `[]` | 社交登录 `['icon','url','label']` |
| `sideImage` | `'auth.jpg'` | 侧栏背景图（**四种写法**，见 docs/auth.md §5） |
| `sideTitle` / `sideText` / `sideList` | 默认文案 / 3 条 | 侧栏文案卡片 |
| `sideVariant` | `'primary'` | 无图时的纯色渐变 |
| `sideOverlay` | `true` | 叠加暗化蒙版保证白字可读 |
| `ajax` | `true` | 表单 `data-xf-remote` 异步提交 |
...
echo XfAdmin::authPage([
    ...
    'type'    => 'sign-in',
    'fields'  => [
        'username' => ['name' => 'username', 'label' => '账号', 'required' => true, 'autofocus' => true],
        'password' => ['name' => 'password', 'type' => 'password', 'label' => '密码', 'required' => true],
    ],
    'submit'    => ['text' => '登 录', 'icon' => 'ti ti-login'],
    'sideImage' => 'https://images.unsplash.com/photo-1521737604893-d14cc237f11d?w=1600',
...
> 分类：**数据 / 业务** · 类：`zxf\XfAdmin\Components\Data\ImportExport`

批量导入/导出工具条（DataTable / 管理页通用）—— 导出为 Bootstrap 下拉（CSV/Excel/JSON 等真链接），导入为带文件域的模态表单。自我包含，不依赖额外前端插件；导出项指向后端导出端点，导入表单 `action` 指向后端导入端点，CSRF 由宿主经 `csrf` 选项注入。
...
| 键 | 默认值 | 说明 |
|----|--------|------|
| `exports` | `[]` | 显式导出项 `[['label','format','url']]`；非空时优先 |
| `export_url` | `''` | 简写基址，配合 `formats` 自动生成导出项（`url = export_url . format`） |
| `formats` | `[]` | 简写格式列表（默认空；使用 `export_url` 简写时必须显式传 `formats`） |
| `import` | `null` | `null` 或 `['url','accept','title']`；`url` 为空则不渲染导入 |
| `export_label` | `'导出'` | — |
| `import_label` | `'导入'` | — |
| `csrf` | `''` | 宿主注入的隐藏域 HTML（如 Laravel `csrf_field()`） |
| `class` | `''` | — |
| `id` | `null` | — |
...
echo XfAdmin::importExport([
    ...
    'import'  => ['url' => '/admin/users/import', 'accept' => '.csv,.xlsx', 'title' => '导入用户'],
    'csrf'    => csrf_field(),   // Laravel：注入 CSRF 隐藏域
]);

// 简写：export_url + formats 自动生成导出项
echo XfAdmin::importExport([
    'export_url' => '/admin/users/export?fmt=',
    'formats'    => ['csv', 'json'],
